...
select  count (*) as emp_cnt, current_title from latest_title group by current_title order by emp_cnt desc;

-- Count the total number of employees per department
with cnt_by_dept as(
select dept_no, count(emp_no) as emp_cnt from dept_emp group by dept_no )
select dept_name, cnt_by_dept.emp_cnt from departments join cnt_by_dept on (departments.dept_no=cnt_by_dept.dept_no) order by emp_cnt desc;
